# Descpa (Declarative Simple Combinator Parser)

`descpa` is a declarative, simple combinator-based parser library. It allows you to build complex parsers by composing
simple rules.

## Basic Usage

The library uses concepts to define rules and productions.

```cpp
#include <fil/descpa/descpa.hh>
#include <fil/descpa/sink.hh>

// Example: checking for a specific character
fil::descpa::match_char<'X'> char_x_check;
fil::descpa::details_::matcher_ctx<fil::descpa::sink::convertor_noop> ctx;

auto res = char_x_check.match(ctx, 'X'); 
// res == fil::descpa::match_result::SUCCESS
```

## String Matching

```cpp
// Check for a fixed string
fil::descpa::match_string<fil::descpa::fixed_string {"CHOCOBO"}> string_check;

// Returns CONTINUE while the string is being matched, and SUCCESS when complete
string_check.match(ctx, 'C'); // CONTINUE
string_check.match(ctx, 'H'); // CONTINUE
// ...
string_check.match(ctx, 'O'); // SUCCESS
```

## Rule Composition

You can compose rules using the `+` operator.

```cpp
// Match "LOL" by composing 'L', 'O', 'L'
auto lol_parser = fil::descpa::match_char<'L'>{} + 
                  fil::descpa::match_char<'O'>{} + 
                  fil::descpa::match_char<'L'>{};

// This creates a tuple_rule that matches the sequence.
```

## Integrating with File Reader

`descpa` is designed to work efficiently with `file_reader`.

```cpp
#include <fil/file/file_reader.hh>
// Use fil::parse with a production and a reader
// auto result = fil::descpa::parse(production, reader);
```
